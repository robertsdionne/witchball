...
void View::DrawScore(const Model &model) const {
  ...
  ofSetColor(color_p1 / 1.0, max(30.0, model.p1_score_alpha * model.p1_score_alpha * 255.0));
  for (int i = 0; i < model.player1_score; ++i) {
    ofPushMatrix();
    ofRect(-kHalfCourtWidth + i * 1.1, -kHalfCourtHeight, 1.0, kCourtHeight);
    ofPopMatrix();
  }
  ofSetColor(color_p2 / 1.0, max(30.0, model.p2_score_alpha * model.p2_score_alpha * 255.0));
...
void View::DrawTriangles(const Model &model) const {
  ...
  ofSetColor(ofColor::black, model.winning_alpha * 255);
  ofRect(-kHalfCourtWidth, -kHalfCourtHeight, kCourtWidth, kCourtHeight);
